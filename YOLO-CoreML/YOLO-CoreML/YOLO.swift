...
class YOLO {
    public static let inputWidth = 416 //416 //608
    public static let inputHeight = 416 // 416 //608
    ...
    let isTiny = true
    let shouldResizeInputImage = true
    let numClasses = 80
    let boxesPerCell = 3
    ...
    public func predict(image: CVPixelBuffer) throws -> [Prediction] {
        ...
        if shouldResizeInputImage {
            let scaledSize = CGSize(width: YOLO.inputWidth, height: YOLO.inputHeight)
            guard let scaledPixelBuffer = image.resized(to: scaledSize) else {
                return []
            }
            inputImage = scaledPixelBuffer
        }
        
        if isTiny {
            if let output = try? tinyModel.prediction(image_input: inputImage) {
            ...
            if let output = try? model.prediction(input_1: inputImage) {
                // The order of output Identity will be random, please double check of outputs order here
